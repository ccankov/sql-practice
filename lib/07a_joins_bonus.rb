...
def rock_superstars
  ...
  execute(<<-SQL)
    SELECT
      artist, COUNT(title) as num_albums
    FROM
      albums
      JOIN
...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      style, COALESCE(AVG(price / c.track_count), 0) as avg_price_track
    FROM
      albums AS a
      JOIN
        styles AS s ON (a.asin = s.album)
      JOIN
        (SELECT
        ...
        FROM
          tracks
        GROUP BY
          album) AS c ON (a.asin = c.album)
    GROUP BY
      style
    ORDER BY
      avg_price_track DESC
    LIMIT
      5
  SQL
end
